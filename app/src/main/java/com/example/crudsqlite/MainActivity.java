...
public class MainActivity extends AppCompatActivity {
    ...
    ArrayList<Integer> arrayListHarga;
    ListView listView;
    Button btnAdd;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                arrayListHarga.add(Integer.parseInt(cursor.getString(0)));
    ...
    public boolean onContextItemSelected(MenuItem item) {
        if (item.getTitle() == "view") {
            ...
            intent.putExtra("MainHarga", Integer.toString(arrayListHarga.get(item.getGroupId())));
            startActivity(intent);
            ...
            dbHelper.deleteData(Integer.toString(arrayListHarga.get(item.getGroupId())));
